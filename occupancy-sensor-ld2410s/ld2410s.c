...
static void *poller_thread(void *arg) {
  ...
  while (!atomic_load_explicit(&s->stop, memory_order_relaxed)) {
    ...
    bool uart_occ =
        atomic_load_explicit(&s->uart_occupied, memory_order_relaxed);
    uint16_t distance =
        (uint16_t)atomic_load_explicit(&s->uart_distance, memory_order_relaxed);
    bool occupied = gpio_occ || uart_occ;

    s->report_cb(occupied, distance, s->report_cb_ud);
    sleep(1);
  }
  return NULL;
}
...
struct LD2410S *ld2410s_init(const struct LD2410S_config *cfg,
  ...
  if (cfg->enable_uart) {
    s->uart = ld2410s_uart_init(cfg->device, cfg->debug, on_uart_report, s,
...
int ld2410s_start_calibration(struct LD2410S *s, uint16_t trigger,
  ...
  if (!s || !s->uart) {
    // Return a dummy OK - we don't have UART, so calibration requests get
    // signaled as OK
    return 0;
  }

  bool room_empty = ld2410s_uart_get_vacant_reports_count(s->uart) >=
                    VACANT_REPORTS_ROOM_EMPTY_THRESHOLD;
  if (!room_empty) {
  ...
  return ld2410s_uart_start_calibration(s->uart, trigger, retention,
                                        duration_secs);
}
