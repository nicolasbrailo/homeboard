...
void on_drm_mgr_updown(void *ud, bool up) {
  struct AmbienceCtx *ctx = ud;
  // This doesn't need a drm mutex because the mutex is held in the render
  // object
  if (up) {
    if (ctx->drm_mgr) {
      fprintf(stderr, "drm_mgr service reported up, but we were already "
                      "connected. Will renew connection.\n");
    ...
    if (!ctx->drm_mgr)
      fprintf(stderr,
              "drm_mgr service reported up, but dbus connection failed\n");
  } else {
...
int main(int argc, char *argv[]) {
  ...
  // Simulate service up callback to connect to DRM. If it fails, we'll connect
  // later
