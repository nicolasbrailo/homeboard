...
static int method_acquire_lease(sd_bus_message *m, void *ud,
                                sd_bus_error *err) {
  (void)ud;
  const char *sender = sd_bus_message_get_sender(m);
  if (!sender)
    return sd_bus_error_set(err, "io.homeboard.Display.Error.NoSender",
  ...
  if (g_lease_track && !sd_bus_track_contains(g_lease_track, sender)) {
    printf("Lease requested by PID %d, but already owned by PID %d\n", req_pid,
           g_lease_pid);
    return sd_bus_error_setf(err, "io.homeboard.Display.Error.LeaseHeld",
  ...
  return sd_bus_reply_method_return(m, "huuuuu", drm_get_dmabuf_fd(g_drm),
                                    fbi->width, fbi->height, fbi->stride,
                                    fbi->format, fbi->size);
}

static int method_release_lease(sd_bus_message *m, void *ud,
                                sd_bus_error *err) {
  (void)ud;
  const char *sender = sd_bus_message_get_sender(m);
  if (!g_lease_track || !sender ||
      !sd_bus_track_contains(g_lease_track, sender)) {
    printf("PID %d requested lease release, but is not the owner (owner is PID "
           "%d)\n",
           sender_pid(m), g_lease_pid);
    return sd_bus_error_set(err, "io.homeboard.Display.Error.NotLeaseHolder",
...
static const sd_bus_vtable g_vtable[] = {
    ...
    SD_BUS_METHOD("AcquireLease", "", "huuuuu", method_acquire_lease,
                  SD_BUS_VTABLE_UNPRIVILEGED),
    SD_BUS_METHOD("ReleaseLease", "", "", method_release_lease,
                  SD_BUS_VTABLE_UNPRIVILEGED),
